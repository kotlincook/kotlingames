...
fun GameBoard<Int?>.moveValuesInRowOrColumn(rowOrColumn: List<Cell>): Boolean {
    ...
    val newValues = oldValues.moveAndMergeEqual { 2 * it }
...
fun GameBoard<Int?>.moveValues(direction: Direction): Boolean {
    return (1..4).fold(false) { acc: Boolean, i: Int ->
        when (direction) {
            Direction.DOWN ->
                acc or moveValuesInRowOrColumn(getColumn(1..4, i).reversed())
            Direction.UP ->
                acc or moveValuesInRowOrColumn(getColumn(1..4, i))
            Direction.LEFT ->
                acc or moveValuesInRowOrColumn(getRow(i, 1..4))
            Direction.RIGHT ->
                acc or moveValuesInRowOrColumn(getRow(i, 1..4).reversed())
        }
    }
}